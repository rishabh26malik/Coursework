...
# Q 2
select Fname from EMPLOYEE as emp, DEPENDENT as dep where emp.ssn=dep.essn and emp.sex=dep.sex and emp.fname=dep.dependent_name; 

select Pnumber from PROJECT where Pname="ProductX" and Dnum=5;

# Q3
select fname, lname
from EMPLOYEE emp, (select Essn 
					from WORKS_ON 
					where hours>5 and Pno in (select Pnumber from PROJECT 
									where Pname="ProductX" and Dnum=5)) tmp
where emp.ssn=tmp.essn and emp.dno=5;

select pnumber, dnum, lname, bdate, address, bdate
from Employee emp, (select mgr_ssn, tmp.dnum, pnumber, dname
		from DEPARTMENT dpt, (select Pnumber,Dnum
		from PROJECT where Plocation="Stafford") tmp) tmp2
        where dpt.Dnumber=tmp2.Dnumber

where tmp2.mgr_ssn=emp.ssn
